...
K-mer解析ではピークが一つなので、BN/NHsdMcwiが**高度に近交化した「実質1倍体系統」**であることが確認され、  
mRatBN7.2で見られたような**PacBio CLR のエラーを“ヘテロ接合”と誤認してハプロタイプが分離されてしまう現象（偽ハプロタイプ分離）**が、本来は起きるべきではなかったことを示している。
...
多種実験動物版「KOzoo」の開発でマウス以外のゲノムアノテーションに触れる機会が増え、とくに近縁のラット対応は重要だと感じていたので、GRCr8は興味深かったです。  

私が見たところでは、mRatBN7.2では転写産物アノテーション数がマウスの約4分の3にとどまり、実際の転写産物数を反映しきれていない印象でした。GRCr8はENSEMBLで既に利用可能でGTFも公開されているので、どれだけ改善されているか楽しみです。  

UCSC Genome Browserはデフォルトが2014年のrn6（RGSC 6.0）で、rn7 (mRatBN7.2)も選べるがカスタムトラックがかなり少ないです。GRCr8対応がいつになるのか気になるところ…  

蛇足ですが、Markdownで数式を書くなら`$$ ... $$`か````math ... ````で囲む方法があるが、個人的には````math ... ````の方が扱いやすかったです。`$$`記法だと改行が崩れやすいので注意です。
