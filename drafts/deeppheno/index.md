...
**予測表現型の有用性**  
DeepPhenoが予測した表現型は、疾患原因遺伝子を特定するための有用な特徴量（シグナル）として十分機能することが示されました。

**配列のみからの原因遺伝子推定**  
特に注目すべき点は、実験的な機能情報を使わず、タンパク質のアミノ酸配列から予測した機能のみを入力としたモデル（DeepPhenoDG）でも、AUROC 0.72という高い性能を示した点です。

これは、  
**「機能が未解明でも、アミノ酸配列が分かっていれば、その遺伝子が関与し得る疾患を予測できる」**  
可能性を示しており、DeepPhenoの実用性を強く示唆しています。
...
**Deepではない**です。  
オントロジーを機械学習に組み込んだことが肝の論文でした。
むしろHPOを扱えるように層を工夫したほうが強いので、`DeepPheno`という名前との乖離を感じました。  
深層学習がバイオインフォマティクスの中で流行り始めていた時代（2020年出版）なので、とりあえずDeepをつけてたほうが目に入りやすい、という気持ちはわかる気がします。  

論文としては非常にお手本になる構成（既存の手法との評価をして、ヒト疾患の予測タスクで占めるといった流れ）で、勉強になりました。  
